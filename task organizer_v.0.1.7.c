...
loadTask(){
	task *lamatask;
	lamatask=(task*)malloc(sizeof(task));
//	strcpy(lamatask->namatask, loadnama);
	lamatask->pointtask = loadpoint;
	lamatask->bulan = loadbulan;
	lamatask->tanggal = loadtanggal;
	lamatask->jam = loadjam;
	lamatask->menit = loadmenit;
...
//		fscanf(loadtask,"%s",loadnama);	
		fscanf(loadtask,"%d",loadpoint);
		fscanf(loadtask,"%d",loadbulan);
		fscanf(loadtask,"%d",loadtanggal);
		fscanf(loadtask,"%d",loadjam);
		fscanf(loadtask,"%d",loadmenit);
		loadTask();		
